    ...
    for i in range(len(folder_names)):
        ...
        for patient_name in os.listdir(current_path):
            ...
            for sample_name in os.listdir(current_patient_path):
                ...
                # Read 32 CSV files per sample
                # List all files in the folder
                all_files = os.listdir(current_sample_path)

                # Sort the files by name (this can be adjusted depending on how you want to sort them)
                all_files.sort()

                # Iterate through the sorted files and process them
                for sensor_idx, file_name in enumerate(all_files):
                    if sensor_idx >= 32:  # Ensure we only process the first 32 files
                        break
                    csv_path = os.path.join(current_sample_path, file_name)
                    data = np.loadtxt(csv_path, delimiter=',', dtype=np.int32)
                    images_hold[sensor_idx].append(data)
