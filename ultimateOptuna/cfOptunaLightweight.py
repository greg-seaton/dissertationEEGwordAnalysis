...
from tensorflow.keras.layers import Input, Conv2D, MaxPool2D, Flatten, Dense, Dropout, Concatenate, Reshape, BatchNormalization
...
saveFolder = os.path.join(current_directory, "savedCFLWmodels")
...
def spectrogram_CNN(trial):
    ...
    for input_layer in inputs:
        # Reshape flat input to 2D (8x12 spectrogram)
        x = Reshape((8, 12, 1))(input_layer)
        
        # Apply same CNN operations as in original
...
# Extract raw data from a CSV instance folder
def load_sample(folder_path):
    ...
    for file in files:
        ...
        # Ensure we have 96 values per CSV file (8x12 spectrogram)
        if len(data) != 96:
            print(f"Warning: {file_path} has {len(data)} values, expected 96")
        sample_data.append(data)
    
    # Check if we have all 32 channels
    if len(sample_data) != 32:
        print(f"Warning: {folder_path} has {len(sample_data)} files, expected 32")
        
    return sample_data

# Goes from folders to raw data to train, test and validate (unchanged except for load_sample)
def NN_prep(folders_path, folders_names):
    ...
    # Convert to list of 32 arrays for the model inputs
...
# Modified NN function that accepts a trial for optimization - kept same as original
...
# Train the final model with best parameters - kept same as original
