...
def CNN_for_data(folders_path, folders_names, no_classes, no_cnn_trained, no_batch_size, momentum_value):
    ...
    for k in range(no_cnn_trained):
        ...
        inputs = [Input(shape=(434, 343, 3)) for eeg in training_images]

        # Creating convolution element for each input
        eeg_cnn_elements = []
        for input in inputs:
            x = Conv2D(filters=3, kernel_size=(5,5), activation='relu', padding='same')(input)
...
folders_path = os.path.join(current_directory, "spectrogramData")
